...
namespace PlatformerPro
{
	...
	{
        bool fileHasBeenUpdated;
        ItemDatabase itemDatabase;
        GameObject bacteriaParent;
        string bacteriaName;
        /// <summary>
		...
		void Start()
		{
            ...
            bacteriaParent = GameObject.Find("End of Level Bacteria Collection");
            bacteriaName = bacteriaParent.transform.GetChild(1).name;
        }

        void Update()
        {
            if (hasHitCharacter == true && fileHasBeenUpdated == false)
            {
                itemDatabase.FetchItemByNameAndSave(bacteriaName);
		...
		virtual protected void DoEventForCollect(Character character)
		{
            if (CollectItem != null)
            {
                OnCollectItem(itemClass, type, character);
            }
            else
            {
